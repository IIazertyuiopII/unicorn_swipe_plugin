...
(function (global, document) {
    ...
    function handleTouchEnd(event) {
        ...
        if (l > min_length && stop && start && stop.time - start.time < 10000) {
            ...
            for (var i = 0; i <= l - min_length; ++i) {
                ...
                seg_types[i] = [0, 0]; /* the segments are classified in one of the four quadrants */
            ...
            for (var i = max_freqs.length - 2; i >= 0; i--) { /* remove duplicates */
                if (previous[0] === max_freqs[i][0]) { /* same direction */
                    if ((previous_type[0] === seg_types[i][0]) && (previous_type[1] === seg_types[i][1]) /* same diagonal */ || Math.abs(max_freqs[i][0]) > 500 /* same vertical */ || (max_freqs[i][0] === 0 && previous_type[0] === seg_types[i][0])) { /* same horizontal */
